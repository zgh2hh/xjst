    ...
    <el-main width="100%">
      <div class="map-wrapper">
        <v-map :zoom='zoom' :center='center' :bounds='bounds' class="map" ref='map'>
          ...
          <v-marker v-for="(item, i) in tracks" :key="i" :lat-lng='item.marker' :icon="getIcon(item.selected)" :style="{'background-color': '#11a'}" :draggable='true' @l-dragend='endDrag($event, item.id)'>
